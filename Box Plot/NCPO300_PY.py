...
for n in range(noiL):
    for r in range(repeat):
        for i in range(6):
            print(i)
            tmp=SimCom(noise_rang[n],noise_rang[n],T,level)
            if (tmp):
                Z[r,n] = tmp
                N[r,n] = i
                break
...
# If yes, do the failed experments more times until success
for i in range(repeat):
    for j in range(noiL):
        if (Z[i,j]==0):
            print(i,j)
            tmp=SimCom(noise_rang[j],noise_rang[j],T,level)
            if (tmp):
                Z[i,j] = tmp
                break
"""

# Write experiment results to Zout.csv
nrmse.to_csv("Zout.csv",header=False,index=False)